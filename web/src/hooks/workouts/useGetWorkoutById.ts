...
/**
 * Hook to fetch a workout by its ID.
 *
 * @param id - The ID of the workout to fetch.
 * @param navigate - The navigation function to redirect the user if needed.
 * @returns An object containing the workout data, loading state, debug info, and a retry function.
 *
 * @example
 * const { workout, isLoading, debugInfo, retry } = useGetWorkoutById(id, navigate);
 */
export const useGetWorkoutById = (id: string | undefined, navigate: NavigateFunction) => {
  ...
  useEffect(() => {
    const loadWorkout = async () => {
      if (!id) {
        logger.error('No workout ID provided');
